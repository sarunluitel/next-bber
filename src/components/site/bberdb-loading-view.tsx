...
const BBER_DB_LOADING_STEPS = [
  "Getting all tables",
  "Getting table column values",
  "Putting it all together",
] as const;

export function BberDbLoadingView() {
  const [activeStepIndex, setActiveStepIndex] = useState(0);

  useEffect(() => {
    const intervalId = window.setInterval(() => {
      setActiveStepIndex((currentIndex) =>
        Math.min(currentIndex + 1, BBER_DB_LOADING_STEPS.length - 1),
      );
    }, 1200);
      ...
      <Card className="border border-[var(--bber-border)] bg-white py-0 shadow-sm">
        <CardHeader className="gap-3 px-6 pt-6">
          ...
          <CardTitle className="font-display text-3xl text-[var(--bber-red)]">
            Loading BBER Data Bank Database
          </CardTitle>
          <p className="max-w-3xl text-sm leading-7 text-[var(--bber-ink)]/76">
            ...
            and loaded table so the data portal opens with the right context.
          </p>
        </CardHeader>
        <CardContent className="grid gap-4 px-6 pb-6 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)]">
          <div className="space-y-3">
            {BBER_DB_LOADING_STEPS.map((step, stepIndex) => {
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {Array.from({ length: 5 }).map((_, index) => (
                <div
                  ...
                  className="h-11 animate-pulse rounded-2xl border border-[var(--bber-border)] bg-[var(--bber-sand)]/75"
                />
              ))}
            </div>
            <div className="min-h-[22rem] animate-pulse rounded-[2rem] border border-[var(--bber-border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(247,242,234,0.88))]" />
          </div>
        </CardContent>
      </Card>
    </div>
